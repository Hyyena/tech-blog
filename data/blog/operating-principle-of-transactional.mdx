...
즉, 무슨 이유에서인지 트랜잭션이 적용되지 않아 영속성 컨텍스트가 생성되지 않고, 더티 체킹이 수행되지 않은겁니다.
...
다시 문제가 발생한 상황을 살펴보면, 위에서 살펴본 것과 같이 아래 코드도 프록시 객체가 생성되지만, 트랜잭션이 적용된 `refreshToken()` 메소드를 클래스 내부에서 직접적으로 호출하고 있기 때문에 프록시 객체를 거쳐서 트랜잭션이 적용된 채 메소드 실행이 이루어지지 않고, 메소드 그 자체만 실행되게 됩니다.
...
public class TokenService {
    ...
    @Transactional
    public void updateUserToken(String tokenId) {
        // 트랜잭션 처리 로직
    }
}
...
public class AuthService {
    ...
    public void refreshToken(String tokenId) {
        // 프록시 객체를 통해 호출됨
        tokenService.updateUserToken(tokenId);
    }
}
```
